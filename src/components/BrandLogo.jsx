 ...
 * stroked monoline, so every variant inherits its color from `currentColor` and
 * scales without a raster fallback.
 *
 * The counter of the E is punched out with a mask instead of being filled with
 * the background color, so the mark sits on any surface — Carbon, Paper, Acid
 * or photography — without a halo.
 */
...
// Punch-out for the counter of the E. Needs a unique id per instance so that
// multiple marks on one page cannot capture each other's mask.
const Counter = ({ id, x, y, width, height }) => (
 ...
 * navigation, footer, credits.
 */
export const BrandSignature = ({ className = '', title = 'Joel van Hees' }) => (
  <svg
    ...
    focusable="false"
  >
    <Counter id="jvh-sig-counter" x="140.32" y="167.44" width="569.36" height="255.05" />
    <g {...strokeProps} mask="url(#jvh-sig-counter)">
      <JStrokes />
      <RestStrokes />
    </g>
  </svg>
...
/**
 * Full lockup — signature plus the "VAN HEES" baseline. Minimum width per the
 * brand book is 42 mm in print / 180 px on screen; below that use the signature.
 */
export const BrandLockup = ({ className = '', title = 'Joel van Hees' }) => (
  <svg
    ...
    focusable="false"
  >
    <Counter id="jvh-lockup-counter" x="140.32" y="167.44" width="569.36" height="298.96" />
    <g {...strokeProps} mask="url(#jvh-lockup-counter)">
      <JStrokes />
      <RestStrokes />
    </g>
    <g {...strokeProps}>
...
/**
 * Compact mark — the Signal J alone, for favicons, avatars and anywhere the
 * full signature would fall under its minimum size.
 */
export const BrandMark = ({ className = '', title = 'Joel van Hees' }) => (
  <svg
